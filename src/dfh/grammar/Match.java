...
public class Match {
	...
	/**
	 * Returns first {@link Match} in a depth-first, left-to-right search that
	 * passes given test.
	 * 
	 * @param t
	 * @return first {@link Match} in a depth-first, left-to-right search that
	 *         passes given test
	 */
	public Match getFirst(MatchTest t) {
		if (children != null) {
			for (Match m : children) {
				Match n = m.getFirst(t);
				if (n != null)
					return n;
			}
		}
	...
	/**
	 * Returns first {@link Match} generated by the given rule. Delegates to
	 * {@link #getFirst(MatchTest)} with an anonymous test generated on each
	 * call. Some efficiency can be gained by using {@link #getFirst(MatchTest)}
	 * with a pre-generated test.
	 * 
	 * @param rule
	 *            rule name
	 * @return first {@link Match} generated by the given rule
	 */
	public Match getFirst(final String rule) {
		return getFirst(new MatchTest() {
			@Override
			public boolean test(Match m) {
